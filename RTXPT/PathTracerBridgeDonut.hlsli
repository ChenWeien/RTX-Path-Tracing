...
sss_sampling_disk_sample( 
run a while(rayQuery.Proceed()) loop,
but need to load surface data before next rayQuery.Proceed()

bool Bridge::SSS_SamplingDiskSample(
    ...
    out float pdf) 
{
    ...
    if (numIntersections > 0) {
        ...
        //payload.objectDescriptorId = wrsObjectDescriptorId;
        payload.triangleId = wrsTriangleID;
        payload.barycentrics = wrsBarycentrics;

        IntersectionInfo(payload, 
            sssSample.position, 
